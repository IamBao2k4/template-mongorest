...
const arraySettings = [
  ...
  {
    title: 'Description allowed file types',
    key: 'meta',
    placeholder: 'Recommend file image',
  },
];

function MultiFileSetting({ parameters, onChange }) {
  return (
      ...
      {arraySettings.map((item, index) => (
        <div
          ...
          <WrapperSetting
            ...
            value={
              item.key === 'image_type' ? parameters.image_type || 'image/*' : parameters[item.key]
            }
            onChange={(val) => {
...
export default MultiFileSetting;
