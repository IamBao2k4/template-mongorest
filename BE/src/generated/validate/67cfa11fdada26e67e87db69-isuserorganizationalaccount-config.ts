export const ISUSERORGANIZATIONALACCOUNT = {
  _id: "67cfa11fdada26e67e87db69",
  title: "is-user-organizational-account",
  entity: [
  "6757b1998659c9e98a2f1e2b"
],
  data: {
  "id": "24ca9183-d2a9-4e42-84b2-0b4c95001e39",
  "rules": [
    {
      "id": "e91bfdc8-3398-4146-b69d-fe3e32fddfc5",
      "field": "user-tenant-level-mapping.user",
      "operator": "=",
      "valueSource": "value",
      "value": "$user-tenant-level-mapping.user:jwt.user@id"
    },
    {
      "id": "6ea3541e-7450-4dd6-9f14-d48ef65acabd",
      "field": "user-tenant-level-mapping.tenant_level",
      "operator": "=",
      "valueSource": "value",
      "value": [
        {
          "_id": "67c928a0f4aa922e5d2a0fa4",
          "title": "organizational user",
          "locale": null,
          "locale_id": null,
          "created_at": "2025-03-06T11:46:24.000Z",
          "created_by": {
            "_id": "6711e8a47b45b2974bd6133c",
            "username": "admin2024@gmail.com",
            "featured_image": [
              {
                "_id": "67c52c92cb2d3f0de049849f",
                "filename": "e2327500-f185-4062-8841-c18ca152384c.jpg",
                "disk": "satellite",
                "mime": "image/jpeg",
                "size": 95662,
                "title": "AVA-AI-Voice-Assistant.jpg",
                "alt": "AVA-AI-Voice-Assistant.jpg",
                "tenant_id": "674028d2611a654e763a73e8",
                "created_by": "6711e8a47b45b2974bd6133c",
                "created_at": "2025-03-03T11:14:10.000Z",
                "updated_at": "2025-03-03T11:14:10.000Z",
                "is_draft": true,
                "__v": 0,
                "path": "https://mgs-storage.sgp1.digitaloceanspaces.com/satellite/e2327500-f185-4062-8841-c18ca152384c.jpg"
              }
            ],
            "first_name": "Không Tên",
            "last_name": "Không Tên."
          },
          "updated_at": "2025-03-06T11:46:24.000Z",
          "updated_by": {
            "_id": "6711e8a47b45b2974bd6133c",
            "username": "admin2024@gmail.com",
            "featured_image": [
              {
                "_id": "67c52c92cb2d3f0de049849f",
                "filename": "e2327500-f185-4062-8841-c18ca152384c.jpg",
                "disk": "satellite",
                "mime": "image/jpeg",
                "size": 95662,
                "title": "AVA-AI-Voice-Assistant.jpg",
                "alt": "AVA-AI-Voice-Assistant.jpg",
                "tenant_id": "674028d2611a654e763a73e8",
                "created_by": "6711e8a47b45b2974bd6133c",
                "created_at": "2025-03-03T11:14:10.000Z",
                "updated_at": "2025-03-03T11:14:10.000Z",
                "is_draft": true,
                "__v": 0,
                "path": "https://mgs-storage.sgp1.digitaloceanspaces.com/satellite/e2327500-f185-4062-8841-c18ca152384c.jpg"
              }
            ],
            "first_name": "Không Tên",
            "last_name": "Không Tên."
          },
          "tenant_id": "67c6ad0ccb2d3f0de04c67eb"
        }
      ]
    }
  ],
  "combinator": "and",
  "not": false
},
  required: [],
  queryMongodb: `{"$and":[{"user-tenant-level-mapping.user":"$user-tenant-level-mapping.user:jwt.user@id"},{"user-tenant-level-mapping.tenant_level":["67c928a0f4aa922e5d2a0fa4"]}]}`,
  locale: null,
  locale_id: null,
  tenant_id: "67c6ad0ccb2d3f0de04c67eb",
  documents: [],
  body: null,
  categories: [],
  headers: null,
  params: null,
} as const;

export type IsuserorganizationalaccountConfig = typeof ISUSERORGANIZATIONALACCOUNT;