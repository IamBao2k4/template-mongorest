 ...
 */
export class MySQLAdapter extends BaseDatabaseAdapter<Pool> {
  ...
  async executeQuery<T = any>(
    ...
    try {
      ...
      try {
        ...
        return this.createResult(data as any, this.getCurrentQuery(), nativeQuery, executionTime);
      } finally {
        connection.release();
      }
    } catch (error) {
  ...
  async getIntanceDb() :Promise<Pool> {
    if (!this.pool) {
      throw error("error")
    }
    return this.pool
