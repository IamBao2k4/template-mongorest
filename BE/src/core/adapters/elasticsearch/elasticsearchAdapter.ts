 ...
 */
export class ElasticsearchAdapter extends BaseDatabaseAdapter<Client> {
  ...
  async executeQuery<T = any>(
    ...
    try {
      const response: any = await this.client.search(nativeQuery);
  ...
  async getIntanceDb(): Promise<Client> {
    if (!this.client) {
      throw error("error")
    }
    return this.client
